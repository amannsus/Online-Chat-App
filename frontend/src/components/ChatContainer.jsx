...
const ChatContainer = () => {
  ...
  const renderMessage = useCallback((msg, index) => {
    ...
    const key = msg._id || `msg_${index}_${senderId}_${msg.createdAt}`;

    return (
      <div key={key} className={`flex ${isCurrentUser ? "justify-end" : "justify-start"} mb-3 px-2 sm:px-4`}>
          ...
          {!isCurrentUser && (
            <div className="size-6 sm:size-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
              {msg.senderId?.profilePic ? (
                <img src={msg.senderId.profilePic} alt={senderName} className="size-6 sm:size-8 rounded-full object-cover" />
              ) : (
                <span className="text-primary font-semibold text-xs">{senderName.charAt(0)}</span>
              )}
            </div>
          )}
          <div className={`px-3 py-2 sm:px-4 sm:py-3 rounded-2xl shadow-sm ${
              isCurrentUser ? "bg-primary text-primary-content rounded-br-md" : "bg-base-300 text-base-content rounded-bl-md"
            }`}>
      ...
      <div className="flex flex-col h-full bg-base-100">
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center p-6 max-w-md mx-auto">
            ...
            <h3 className="text-xl font-semibold text-base-content/70 mb-2">Welcome to Yap! 🤗</h3>
            <p className="text-base-content/50">Select a contact or group to start chatting</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-base-100 relative w-full">
      {/* Header */}
      <div className="p-3 sm:p-4 border-b border-base-300 bg-base-200/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <button onClick={handleBackButton} className="btn btn-ghost btn-sm btn-square md:hidden">
              <ArrowLeft className="size-5" />
            </button>

            <div className="size-10 sm:size-12 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
              {chatAvatar ? (
                <img src={chatAvatar} alt={chatName} className="size-10 sm:size-12 rounded-full object-cover" />
              ) : (
                <span className="text-primary font-semibold text-sm sm:text-base">
                  {isGroupChat ? <Users className="size-5 sm:size-6" /> : (chatName?.charAt(0) || 'U')}
                </span>
              )}
            </div>

            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-sm sm:text-base truncate">{chatName}</h3>
              <p className="text-xs sm:text-sm text-base-content/70 truncate">
                {isGroupChat ? `${currentChat.members?.length || 0} members` : (currentChat.online ? 'Online' : 'Offline')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            {isGroupChat && (
              <button className="btn btn-ghost btn-sm btn-square" title="Group Settings">
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden bg-gradient-to-b from-base-100 to-base-200/30"
      ...
      {selectedFile && (
        <div className="px-4 py-3 border-t border-base-300 bg-base-200/50">
          <div className="flex items-center gap-3 p-3 bg-base-100 rounded-lg border border-base-300">
            {filePreview ? (
              <img src={filePreview} alt="Preview" className="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded-lg" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-base-content/70">{formatFileSize(selectedFile.size)}</p>
            </div>
            <button onClick={removeSelectedFile} className="btn btn-ghost btn-sm btn-square flex-shrink-0">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <div
        className="p-3 sm:p-4 border-t border-base-300 bg-base-200/50 backdrop-blur-sm"
      ...
      >
        ...
        <div
          ...
          className={`absolute bottom-16 right-4 z-20 bg-base-200 shadow-xl rounded-xl p-2 border border-base-300 transform transition-all duration-200 ${
            showAttachMenu ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-2 pointer-events-none'
          }`}
        >
          <button onClick={() => cameraInputRef.current?.click()} className="flex items-center gap-3 w-full p-3 hover:bg-base-300 rounded-lg text-left transition-colors">
            <Camera className="w-5 h-5 text-primary" />
            <span className="text-sm font-medium">Camera</span>
          </button>
          <button onClick={() => imageInputRef.current?.click()} className="flex items-center gap-3 w-full p-3 hover:bg-base-300 rounded-lg text-left transition-colors">
            <Image className="w-5 h-5 text-secondary" />
            <span className="text-sm font-medium">Photo</span>
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-3 w-full p-3 hover:bg-base-300 rounded-lg text-left transition-colors">
          ...
          <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx,.txt,.xls,.xlsx,.ppt,.pptx" onChange={handleFileSelect} className="hidden" />
        </div>

        <div className="flex items-end gap-2">
          <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="btn btn-ghost btn-sm btn-square flex-shrink-0" type="button">
